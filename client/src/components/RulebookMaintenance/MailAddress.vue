  ...
  <v-container fluid pl-0 pr-0 pb-0>
    <v-data-table :headers="headers" :items="items" :search="search" dense>
      ...
      <template v-slot:item.action="{ item }">
        <v-btn @click="editDialog(item.EmailAddressID)" icon small color="blue">
        ...
        <v-btn
          ...
          small
          dark
          @click="deleteRecord(item.EmailAddressID)"
        ...
        <v-btn
          ...
          small
          dark
          @click="restoreRecord(item.EmailAddressID)"
          v-else
        >
    ...
    <v-dialog v-model="dialog" max-width="600" persistent>
      <v-card>
        ...
        <v-row no-gutters dense class="mt-3 pa-2">
          <v-col cols="12">
            <v-text-field
              v-model="editItems.EmailAddress"
              label="Email Address"
              outlined
              dense
            ></v-text-field>
...
export default {
  ...
  methods: {
    getRecord() {
      console.log('aaaaaaaaaaaaaa',this.api)
      let url = `${this.api}get/emailaddress`;
    ...
    deleteRecord(index) {
      let url = `${this.api}delete/emailaddress/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    restoreRecord(index) {
      let url = `${this.api}restore/emailaddress/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    saveRecord() {
      if (!this.editItems.EmailAddress) {
        Swal.fire({
          position: "center",
          icon: "error",
      ...
      if (checkExistItems.includes(this.editItems.EmailAddress)) {
        Swal.fire({
          position: "center",
          icon: "error",
      ...
      if (this.saveToggle == "ADD") {
        this.loading = true;
        let url = `${this.api}add/emailaddress`;
        ...
        this.loading = true;
        let url = `${this.api}update/emailaddress`;
        axios.post(url, this.editItems).then(() => {
          this.loading = false;
          this.dialog = false;
          Swal.fire({
