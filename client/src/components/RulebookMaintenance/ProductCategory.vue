  ...
  <v-container fluid pl-0 pr-0 pb-0>
    <v-data-table :headers="headers" :items="items" :search="search" dense>
      ...
      <template v-slot:item.action="{ item }">
        <v-btn
          @click="editDialog(item.ProductCategoryID)"
          icon
          small
          color="blue"
        >
        ...
        <v-btn
          ...
          small
          dark
          @click="deleteRecord(item.ProductCategoryID)"
        ...
        <v-btn
          ...
          small
          dark
          @click="restoreRecord(item.ProductCategoryID)"
          v-else
        >
    ...
    <v-dialog v-model="dialog" max-width="600" persistent>
      <v-card>
        ...
        <v-row no-gutters dense class="mt-3 pa-2">
          <v-col cols="12">
            <v-text-field
              v-model="editItems.ProductCategory"
...
export default {
  ...
  methods: {
    getRecord() {
      let url = `${this.api}get/ProductCategory`;
    ...
    addDialog() {
      this.saveToggle = "ADD";
      this.dialog = true;
      this.editItems = { ProductCategory: "" };
    },
    ...
    deleteRecord(index) {
      let url = `${this.api}delete/ProductCategory/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    restoreRecord(index) {
      let url = `${this.api}restore/ProductCategory/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    saveRecord() {
      if (!this.editItems.ProductCategory) {
        Swal.fire({
          width: 500,
          position: "center",
          icon: "error",
          title: "Please input Product Category !",
      ...
      if (checkExistItems.includes(this.editItems.ProductCategory)) {
        Swal.fire({
          width:500,
          position: "center",
          icon: "error",
          title: "Product Category already exist !",
          showConfirmButton: false,
          timer: 800,
        });
        return;
      ...
      if (this.saveToggle == "ADD") {
        this.loading = true;
        let url = `${this.api}add/ProductCategory`;
        ...
        this.loading = true;
        let url = `${this.api}update/ProductCategory`;
        axios.post(url, this.editItems).then(() => {
          this.loading = false;
          this.dialog = false;
          Swal.fire({
