  ...
  <v-container fluid pl-0 pr-0 pb-0>
    <v-data-table :headers="headers" :items="items" :search="search" dense>
      ...
      <template v-slot:item.action="{ item }">
        <v-btn
          @click="editDialog(item.ClassificationID)"
          icon
          small
          color="blue"
        >
        ...
        <v-btn
          ...
          small
          dark
          @click="deleteRecord(item.ClassificationID)"
        ...
        <v-btn
          ...
          small
          dark
          @click="restoreRecord(item.ClassificationID)"
          v-else
        >
    ...
    <v-dialog v-model="dialog" max-width="600" persistent>
      <v-card>
        ...
        <v-row no-gutters dense class="mt-3 pa-2">
          <v-col cols="12">
            <v-text-field
              v-model="editItems.Classification"
...
export default {
  ...
  methods: {
    getRecord() {
      let url = `${this.api}get/Classification`;
    ...
    deleteRecord(index) {
      let url = `${this.api}delete/Classification/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    restoreRecord(index) {
      let url = `${this.api}restore/Classification/${index}/${this.updatedBy}`;
      axios.post(url).then(() => {
        this.getRecord();
      });
    },
    ...
    saveRecord() {
      if (!this.editItems.Classification) {
        Swal.fire({
          position: "center",
          icon: "error",
          title: "Please input Classification !",
      ...
      if (checkExistItems.includes(this.editItems.Classification)) {
        Swal.fire({
          position: "center",
          icon: "error",
      ...
      if (this.saveToggle == "ADD") {
        this.loading = true;
        let url = `${this.api}add/Classification`;
        ...
        this.loading = true;
        let url = `${this.api}update/Classification`;
        axios.post(url, this.editItems).then(() => {
          this.loading = false;
          this.dialog = false;
          Swal.fire({
